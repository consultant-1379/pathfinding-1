...
import com.ericsson.eniq.model.CreateView;
...
public abstract class AbstractCreateViewsAction {
 ...
 protected void mergeFreeMarkerTemplateAndCreateView(final String template, final RockFactory dbForCreatingView,
	     final String infoStringForLogging, final HashMap<String,Object> freeMarkerContext) throws Exception, SQLException {
	   final StringWriter sqlWriter = new StringWriter();
	   Template freeMarkerTemplate = null;
	  // freeMarkerTemplate = freeMarkerConfigurer.getConfiguration().getTemplate("createview.ftl");
	   freeMarkerTemplate = freeMarkerConfigurer.getConfiguration().getTemplate(template);
	   freeMarkerTemplate.process(freeMarkerContext, sqlWriter);
	   
	   if (!sqlWriter.toString().equals("")) {
	     attemptToCreateView(sqlWriter.toString(), dbForCreatingView);
	   } else {
	     log.warning("Freemarker failed for " + infoStringForLogging + " ");
	     throw new Exception("Freemarker failed for " + infoStringForLogging + " ");
	   }
	 }
