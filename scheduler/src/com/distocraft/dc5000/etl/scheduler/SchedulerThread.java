...
public class SchedulerThread extends Thread {
	...
	public SchedulerThread(final long pollInterval, final int penaltyWait,
			final String engineURL, final long reConnectTime, final String url,
	...
	private synchronized void triggerSet(final IScheduleTrigger sTrigger,
			Meta_schedulings schedule, final ITransferEngineRMI engine,
		...
		try {
			...
			engine.execute(schedule.getRockFactory().getDbURL(), schedule.getRockFactory().getUserName(), 
			...
			if (flag) {
				try {
					...
					ITransferEngineRMI retryEngine = (ITransferEngineRMI) Naming.lookup(RmiUrlFactory.getInstance().getEngineRmiUrl());
					try {
						boolean isInit = retryEngine.isInitialized();
						boolean isRef = retryEngine.isCacheRefreshed();
						schedule = sTrigger.getSchedule();
						log.info("Catch Block schedule object "+schedule);
						if (!isInit || !isRef) {
					...
					if (retryTriggerCount <= 3) {
						...
						Thread.sleep(60000);
						triggerSet(sTrigger, schedule, retryEngine, "", tpname);
					} else {
