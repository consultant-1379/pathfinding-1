...
public class TPInstallOrderer  {
    ...
    public int execute()  {
		...
		try {
			...
			this.dwhrepRockFactory=getdb.createDwhrepRockFactory(etlrepRockFactory);
	 ...
	 * @throws Exception 
	 */
	private boolean checkTechPackInstalled(String techPackName, TPEntry tp0, final Vector<Tpactivation> tpActivationVect)
			throws Exception {
		boolean flag=false;
		...
		if (datainterfaceSet.size() > 0) {
			...
			if (rstateCompResultforIntf == 0) {
				...
				if (buildNumberInteger > installedBuildNumberInteger) {
					...
					logger.info("Newer or the same version b" + installedIntfBuild
				...
				logger.info("Older tech pack version " + installedIntfRstate
				...
				// This tech pack is older than than the version in
				// database.
				logger.info("Newer version " + installedIntfRstate
	...
	/*protected Map<String, String> getDatabaseConnectionDetails() throws Exception {
		...
		try {
			props = new ETLCServerProperties();
					...
					throw new Exception("Could not read ETLCServer.properties", e);
		}
