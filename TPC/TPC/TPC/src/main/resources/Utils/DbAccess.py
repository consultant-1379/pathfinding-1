...
class DbAccess(object):
    ...
    dbDict['dwhdb']  = {'port': ':2640/dwhdb',  'uid': 'dc',     'pid':'dc',    'driver':'com.sybase.jdbc4.jdbc.SybDriver', 'url':'jdbc:sybase:Tds:DBpath:2640/dwhdb',    'cnxn': None, 'crsr':[]} 
    dbDict['dwhrep'] = {'port': ':2641/dwhrep', 'uid': 'dwhrep', 'pid':'dwhrep','driver':'com.sybase.jdbc4.jdbc.SybDriver', 'url':'jdbc:sybase:Tds:DBpath:2641/dwhrep',  'cnxn': None, 'crsr':[]} 
    dbDict['etlrep'] = {'port': ':2641/etlrep', 'uid': 'etlrep', 'pid':'etlrep','driver':'com.sybase.jdbc4.jdbc.SybDriver', 'url':'jdbc:sybase:Tds:DBpath:2641/etlrep',    'cnxn': None, 'crsr':[]} 
    dbDict['DBA']    = {'port': ':2641/dwhrep', 'uid': 'dba',    'pid':'sql',   'driver':'com.sybase.jdbc4.jdbc.SybDriver', 'url':'jdbc:sybase:Tds:DBpath:2641/dwhrep',    'cnxn': None, 'crsr':[]} 
    dbDict['HSQL']   = {'port': '',             'uid': 'sa',     'pid':'',    'driver':'org.hsqldb.jdbcDriver',           'url':'jdbc:hsqldb:file:DBpath;hsqldb.lock_file=false;hsqldb.sqllog=3;hsqldb.applog=3',              'cnxn': None, 'crsr':[]} 

    def __init__(self, serverAddr=None, dbname=None, dbPass=None, dbDriver=None):
        ...
        if serverAddr is not None and dbname is not None:
            DbAccess.initCount += 1
            
            if dbPass is not None:
                for dbName,passDict in dbPass.iteritems():
                    (self.dbDict[dbName])['pid'] = passDict
    ...
    def getConnectionProperties(self, dbname):
        return DbAccess.dbDict[dbname]
    ...
    def _connect(self):
        ...
        if cnxn is None or cnxn.closed:
            ...
            try:
                cnxnStr = DbAccess.dbDict[self.dbname]['url'].replace('DBpath' , DbAccess.serverAddr)                
                cnxn = zxJDBC.connect(cnxnStr, DbAccess.dbDict[self.dbname]['uid'], DbAccess.dbDict[self.dbname]['pid'], DbAccess.dbDict[self.dbname]['driver'])
